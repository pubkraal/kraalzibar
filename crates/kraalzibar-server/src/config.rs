...
"#
    ...
    #[test]
    fn default_config_has_max_expand_results() {
        let config = AppConfig::default();
    ...
    fn to_engine_config_includes_max_expand_results() {
        let mut config = AppConfig::default();
        config.engine.max_expand_results = 5_000;

        let engine_config = config.to_engine_config();
        assert_eq!(engine_config.max_expand_results, 5_000);
    }
    ...
    fn env_override_max_expand_results() {
        ...
        let env = |key: &str| -> Option<String> {
            match key {
                "KRAALZIBAR_ENGINE_MAX_EXPAND_RESULTS" => Some("20000".to_string()),
                _ => None,
            }
        };
        config.apply_env_overrides_with(env);

        assert_eq!(config.engine.max_expand_results, 20_000);
    }
